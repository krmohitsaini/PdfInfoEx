...
def call_lmstudio(
    ...
    """Send a request to a locally running LM Studio server (OpenAI-compatible API).

    The ``api_key`` parameter is repurposed to hold the server base URL
    (e.g. ``http://localhost:1234/v1``).  No real API key is required.
    """
    from openai import OpenAI

    base_url = api_key or "http://localhost:1234/v1"
    ...
    # Omit response_format — local models may not support json_object mode.
