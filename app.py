...
with st.sidebar:
    ...
    provider = st.selectbox("Provider", ["OpenAI", "Gemini", "Anthropic", "LM Studio"])

    if provider == "LM Studio":
        api_key = st.text_input(
            "Server URL",
            value="http://localhost:1234/v1",
            help="LM Studio local API endpoint. Start the server in LM Studio under Developer → Start Server.",
        )
        lmstudio_model = st.text_input(
            "Model name",
            placeholder="e.g. llama-3.2-3b-instruct",
            help="The identifier of the model currently loaded in LM Studio.",
        )
    else:
...
if uploaded_file is not None and (
    ...
    if provider == "LM Studio" and doc_type == "Scanned":
        st.warning(
            "Most local models do not support vision. "
            "Load a vision-capable model in LM Studio (e.g. llava) before querying a scanned PDF."
        )

    if st.button("🧠 Query with LLM", type="primary", use_container_width=True, disabled=not can_query):
        if provider == "LM Studio":
            if not lmstudio_model:
                st.error("Please enter the model name loaded in LM Studio.")
                st.stop()
